 const express = require('express');
  ...
  app.post('/api/consume', async (req, res) => {
      try {
          console.log('⚠️ Legacy endpoint /api/consume called');
          console.log('   Request data:', JSON.stringify(req.body, null, 2));

          const legacyData = req.body;

          // Return the EXACT format Android app expects
          const legacyResponse = {
              success: true,
              message: 'Credits consumed successfully',
              creditsUsed: legacyData.creditsUsed || 1, // REQUIRED field for Android
              userId: legacyData.userId || 'legacy_user',
              deviceId: legacyData.deviceFingerprint?.substring(0, 16) || 'legacy_device',
              updated: true,
              serverTimestamp: Date.now(),
              serverVersion: '2.0.0-legacy-compat'
          };

          console.log('✅ Legacy consume response:', JSON.stringify(legacyResponse, null, 2));
          res.status(200).json(legacyResponse);

      } catch (error) {
          console.error('❌ Legacy consume endpoint error:', error);

          // Return error in expected format
          const errorResponse = {
              success: false,
              message: 'Failed to consume credits',
              creditsUsed: 0, // REQUIRED field for Android
              error: process.env.NODE_ENV === 'production' ? 'Internal server error' : error.message,
              serverTimestamp: Date.now()
          };

          console.log('❌ Legacy consume error response:', JSON.stringify(errorResponse, null, 2));
